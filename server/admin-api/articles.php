...
try{
    ...
    $lock=kalite_filo_admin_lock_article_store();try{$drafts=kalite_filo_admin_article_drafts();$article=kalite_filo_admin_normalize_article($body);kalite_filo_admin_assert_article_media_reference($article,kalite_filo_admin_article_media_records());$drafts[]=$article;kalite_filo_admin_assert_article_uniqueness($drafts,$records);kalite_filo_admin_write_article_revision('create',$article);kalite_filo_admin_write_article_drafts($drafts);}finally{kalite_filo_admin_unlock_article_store($lock);}kalite_filo_admin_audit('article_create','success',['id'=>$article['id'],'revision'=>$article['revision'],'categoryId'=>$article['categoryId']]);kalite_filo_admin_json(['article'=>$article],201);
