    ...
    >
      ...
      >
        <header className="flex items-center justify-between gap-4 bg-brand-navy px-5 py-5 text-text-inverse sm:px-7">
          ...
          {hasSavedChoice ? (
            <button
              aria-label="Gizlilik tercihlerini kapat"
        ...
        {view === "summary" ? (
          <div className="p-5 sm:p-7">
            <div className="space-y-4 text-body leading-relaxed text-text-secondary" id="privacy-preferences-description">
              <p>
                Kalite Filo internet sitesinde, sayfanın beklenen şekilde çalışması
                ve tercihlerinizi hatırlamak için tarayıcı depolama teknolojileri
                kullanılır. İsteğe bağlı üçüncü taraf içerikler yalnızca izin
                vermeniz halinde yüklenir. Kullanılan teknolojiler, amaçları ve
                saklama süreleri hakkında ayrıntılı bilgi için {" "}
                <a
                  className="font-semibold text-corporate-blue underline underline-offset-4"
                  href="/documents/kalite-filo-cerez-politikasi.pdf"
                  rel="noreferrer"
                  target="_blank"
                >
                  Çerez Politikası’nı
                </a>{" "}
                inceleyebilirsiniz. İsteğe bağlı teknolojileri kabul edebilir,
                reddedebilir veya tercihlerinizi ayrı ayrı yönetebilirsiniz.
              </p>
            </div>
          ...
          <div className="p-5 sm:p-7">
            <p className="text-body text-text-secondary" id="privacy-preferences-description">
              Zorunlu depolama her zaman açıktır. Diğer tercihinizi dilediğiniz
              zaman footer’daki bağlantıdan değiştirebilirsiniz.
            </p>
            <div className="mt-6 space-y-3">
              <PreferenceRow checked description="Sitenin temel işlevlerinin çalışması ve açıkça oluşturduğunuz tercihlerin hatırlanması için gereklidir." disabled label="Zorunlu Teknolojiler" />
              <PreferenceRow checked={functional} description="Tercihinizle etkinleştirilen isteğe bağlı üçüncü taraf site işlevlerini sağlar." label="İşlevsellik Teknolojileri" onChange={setFunctional} />
              <PreferenceRow checked={false} description="Sitede performans veya analitik izleme teknolojisi şu anda kullanılmamaktadır." disabled label="Performans ve Analitik" />
              <PreferenceRow checked={false} description="Sitede kişiselleştirilmiş reklam veya pazarlama izleme teknolojisi şu anda kullanılmamaktadır." disabled label="Reklam ve Pazarlama" />
            </div>
            <div className="mt-7 grid gap-3 sm:grid-cols-3">
              <button className={getActionClassName({ fullWidth: true, size: "primary", variant: "outline" })} onClick={() => persist(false)} type="button">Tümünü Reddet</button>
              <button className={getActionClassName({ fullWidth: true, size: "primary", variant: "secondary" })} onClick={() => persist(functional)} type="button">Seçimleri Onayla</button>
              <button className={getActionClassName({ fullWidth: true, size: "primary", variant: "primary" })} onClick={() => persist(true)} type="button">Tümünü Onayla</button>
            </div>
          </div>
        )}
      </div>
