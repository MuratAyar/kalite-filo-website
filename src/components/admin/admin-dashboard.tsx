...
export type DashboardData = {
  ...
  metrics: { activeVehicles: number; vehicleGrowth: number; totalVehicles: number; draftVehicles: number; featuredVehicles: number; articles: number; articleGrowth: number; draftArticles: number; pendingContent: number; newsletterContacts: number; approvedMarketingConsents: number; newsletterGrowth: number; iysPending: number; unsubscribed: number };
...
function MetricCard({ label, value, note, icon, tone, onClick }: { label: string; value: number; note: ReactNode; icon: Parameters<typeof Icon>[0]["name"]; tone: string; onClick: () => void }) {
  return <button aria-label={`${label} bölümünü aç`} className="rounded-card border border-border-subtle bg-surface-card p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-corporate-blue hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-corporate-blue" onClick={onClick} type="button">
...
function Growth({ value, allTime = false }: { value: number; allTime?: boolean }) {
  return <span className="inline-flex items-center gap-1 font-semibold text-success"><svg aria-hidden="true" className="size-3.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 16 16"><path d="m2 11 4-4 3 3 5-6"/><path d="M10 4h4v4"/></svg>{allTime ? `${value} toplam` : `+${value} seçili dönemde`}</span>;
}
...
export function AdminDashboard({ data, loading, error, range, environment, onRangeChange, onShowLogs, onOpenVehicles, onOpenArticles, onOpenSubscribers, onOpenDrafts, onOpenIys }: { data: DashboardData | null; loading: boolean; error: string; range: DashboardRange; environment: "production" | "staging"; onRangeChange: (range: DashboardRange) => void; onShowLogs: () => void; onOpenVehicles: () => void; onOpenArticles: () => void; onOpenSubscribers: () => void; onOpenDrafts: () => void; onOpenIys: () => void }) {
  return <section className="mt-8">
    ...
    {data ? <>
      <div className={`mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5 ${loading ? "opacity-60" : ""}`}>
        <MetricCard icon="car" label="Yayındaki Araçlar" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.vehicleGrowth}/><p>{data.metrics.totalVehicles} kayıtlı araç · %{data.metrics.totalVehicles ? Math.round(data.metrics.activeVehicles / data.metrics.totalVehicles * 100) : 0} aktif</p></div>} onClick={onOpenVehicles} tone="bg-emerald-100 text-success" value={data.metrics.activeVehicles}/>
        <MetricCard icon="draft" label="Yayındaki Bloglar" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.articleGrowth}/><p>Filo Rehberi içerikleri</p></div>} onClick={onOpenArticles} tone="bg-blue-100 text-corporate-blue" value={data.metrics.articles}/>
        <MetricCard icon="contact" label="Aktif Bülten Kişileri" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.newsletterGrowth}/><p>{data.metrics.newsletterContacts} benzersiz kayıt</p></div>} onClick={onOpenSubscribers} tone="bg-orange-100 text-orange-dark" value={data.metrics.approvedMarketingConsents}/>
        <MetricCard icon="draft" label="Taslak İçerikler" note={<>{data.metrics.draftArticles} rehber · {data.metrics.draftVehicles} araç</>} onClick={onOpenDrafts} tone="bg-violet-100 text-violet-700" value={data.metrics.pendingContent}/>
        <MetricCard icon="pending" label="İYS Bekleyenler" note={<>İYS aktarımı veya onayı bekleyen kişiler</>} onClick={onOpenIys} tone="bg-sky-100 text-sky-700" value={data.metrics.iysPending}/>
      </div>
