...
export function AdminDashboard({ data, loading, error, range, environment, onRangeChange, onShowLogs }: { data: DashboardData | null; loading: boolean; error: string; range: DashboardRange; environment: "production" | "staging"; onRangeChange: (range: DashboardRange) => void; onShowLogs: () => void }) {
  return <section className="mt-8">
    ...
    {data ? <>
      <div className={`mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4 ${loading ? "opacity-60" : ""}`}>
        <MetricCard icon="car" label="Yayındaki Araçlar" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.vehicleGrowth}/><p>{data.metrics.totalVehicles} kayıtlı araç · %{data.metrics.totalVehicles ? Math.round(data.metrics.activeVehicles / data.metrics.totalVehicles * 100) : 0} aktif</p></div>} tone="bg-emerald-100 text-success" value={data.metrics.activeVehicles}/>
        <MetricCard icon="draft" label="Yayındaki Bloglar" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.articleGrowth}/><p>Filo Rehberi içerikleri</p></div>} tone="bg-blue-100 text-corporate-blue" value={data.metrics.articles}/>
        <MetricCard icon="contact" label="Aktif Bülten Kişileri" note={<div className="space-y-1"><Growth allTime={range === "all"} value={data.metrics.newsletterGrowth}/><p>{data.metrics.newsletterContacts} benzersiz kayıt</p></div>} tone="bg-orange-100 text-orange-dark" value={data.metrics.approvedMarketingConsents}/>
