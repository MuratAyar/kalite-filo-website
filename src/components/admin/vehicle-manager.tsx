...
export function VehicleManager({ csrfToken, draftOnly }: { csrfToken: string; draftOnly: boolean }) {
  ...
  async function removeImage(mediaId: string) {
    ...
    const response = await fetch("/admin-api/media-delete.php", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json", "X-CSRF-Token": csrfToken }, body: JSON.stringify({ vehicleId: editing.id, mediaId }) });
    if (response.ok) {
      const galleryMedia = (editing.galleryMedia ?? (editing.draftMedia ? [editing.draftMedia] : [])).filter((media) => media.id !== mediaId);
      setEditing({ ...editing, galleryMedia, draftMedia: galleryMedia[0] ?? null });
      await load();
    } else setError("Görsel silinemedi.");
  }

  async function openEditor(vehicle: Vehicle) {
    setDirty(false); setEditing(vehicle); setCreating(false); setRevisions([]); setShowAllRevisions(false); setRevisionsLoading(true);
    try {
  ...
  return <section className="mt-8">
    <div className="flex justify-between gap-4"><h2 className="text-heading-md">{draftOnly ? "Draft Araçlar" : "Yayındaki Araçlar"}</h2><button className="rounded-control bg-accent-orange px-5 font-semibold" onClick={() => { setDirty(false); setEditing(null); setCreating(true); }}>Yeni Araç</button></div>
    ...
    {(editing || creating) ? <div className="fixed inset-0 z-50 overflow-y-auto bg-brand-navy/75 p-3" onMouseDown={(event) => { if (event.target === event.currentTarget) closeEditor(); }}><form className="mx-auto my-5 max-w-6xl rounded-card bg-page p-5 sm:p-7" onInput={() => setDirty(true)} onSubmit={save}><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-corporate-blue">Araç yönetimi</p><h3 className="mt-1 text-heading-md">{editing ? "Aracı Düzenle" : "Yeni Araç"}</h3></div><button className="rounded-control border border-border-control px-4 py-2 font-semibold" type="button" onClick={closeEditor}>Kapat</button></div><div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <fieldset className="sm:col-span-2 grid gap-3 rounded-card border p-4 sm:grid-cols-2"><legend>Araç galerisi</legend><p className="sm:col-span-2 text-sm text-text-secondary">Yeni görseller galeri sonuna eklenir. İlk görsel kapak görselidir; en fazla 20 görsel kullanılabilir.</p><input accept="image/jpeg,image/png,image/webp" className="sm:col-span-2" name="image" type="file"/>{[['alt', 'Alternatif metin'], ['creator', 'Oluşturan'], ['sourcePage', 'Kaynak sayfası URL'], ['licenseName', 'Lisans adı'], ['licenseUrl', 'Lisans URL']].map(([name, label]) => <label key={name}>{label}<input className={fieldClass} name={name}/></label>)}{editing ? <div className="sm:col-span-2 grid gap-3 sm:grid-cols-3">{(editing.galleryMedia ?? (editing.draftMedia ? [editing.draftMedia] : [])).map((media, index) => <div className="overflow-hidden rounded-control border bg-white" key={media.id}><img alt={media.alt} className="aspect-video w-full object-cover" src={`/admin-api/media-file.php?id=${media.id}`}/><div className="flex items-center justify-between gap-2 p-2"><span className="text-xs font-semibold">{index === 0 ? "Kapak" : `${index + 1}. görsel`}</span><div className="flex gap-2"><a className="text-xs underline" href={`/admin-api/media-file.php?id=${media.id}&download=1`}>İndir</a><button className="text-xs text-error underline" onClick={() => void removeImage(media.id)} type="button">Sil</button></div></div></div>)}</div> : null}</fieldset>
    </div><button className="mt-6 min-h-12 w-full rounded-control bg-accent-orange font-bold">Kaydet</button>{editing ? <section className="mt-6 rounded-card bg-surface-muted p-4"><h4 className="font-bold">Değişiklik geçmişi</h4>{revisionsLoading ? <p className="mt-2 text-sm">Yükleniyor…</p> : revisions.length ? <ol className="mt-3 space-y-3">{(showAllRevisions ? revisions : revisions.slice(0, 2)).map((revision) => <li className="rounded-control bg-white p-3 text-sm" key={revision.id}><div className="flex flex-wrap justify-between gap-2"><strong>{revision.action === "create" ? "Araç oluşturuldu" : "Araç güncellendi"}</strong><time dateTime={revision.timestamp}>{new Date(revision.timestamp).toLocaleString("tr-TR")}</time></div><p className="mt-1 text-text-secondary">Değişen alanlar: {revision.changedFields.join(", ") || "—"}</p>{revision.priceAmountMinor !== null ? <p className="mt-1">Fiyat: {new Intl.NumberFormat("tr-TR").format(revision.priceAmountMinor / 100)} TL + KDV / ay</p> : null}</li>)}</ol> : <p className="mt-2 text-sm text-text-secondary">Henüz kayıtlı bir değişiklik yok.</p>}{revisions.length > 2 ? <button className="mt-3 text-sm font-semibold text-corporate-blue hover:underline" onClick={() => setShowAllRevisions((value) => !value)} type="button">{showAllRevisions ? "Daha Az Göster" : `Daha Fazla Göster (${revisions.length - 2})`}</button> : null}</section> : null}</form></div> : null}
  </section>;
}
