...
export function VehicleManager({ csrfToken, draftOnly }: { csrfToken: string; draftOnly: boolean }) {
  ...
  return <section className="mt-8">
    <div className="flex justify-between gap-4"><h2 className="text-heading-md">{draftOnly ? "Draft Araçlar" : "Tüm Araçlar"}</h2><button className="rounded-control bg-accent-orange px-5 font-semibold" onClick={() => { setDirty(false); setEditing(null); setCreating(true); }}>Yeni Araç</button></div>
