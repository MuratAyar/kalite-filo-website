...
export function VehicleOfferControls(props: Props) {
  ...
  function addToCart(redirectToCart = false) {
    ...
    if (redirectToCart) {
      window.location.assign("/teklif-al/?form=sepet");
      return;
    }
    setAdded(true);
    ...
    <>
      <div className="mt-6 grid grid-cols-2 gap-4">
        <label className="grid min-w-0 gap-2 text-xs font-semibold text-text-primary sm:text-label">
          <span className="whitespace-nowrap">Kiralama Süresi (Ay)<span aria-hidden="true">*</span></span>
        ...
        <label className="grid min-w-0 gap-2 text-xs font-semibold text-text-primary sm:text-label">
          <span className="whitespace-nowrap">Yıllık Kilometre<span aria-hidden="true">*</span></span>
      ...
      <VehicleListPrice className="mt-6" listPrice={props.listPrice} offerPanel />
      <p className="mt-3 text-xs leading-5 text-text-secondary">Liste fiyatı bağlayıcı teklif veya araç bulunabilirliği garantisi değildir.</p>
      <div className="mt-6 grid gap-3">
        <Button data-vehicle-detail-action="quote" fullWidth onClick={() => addToCart(true)} size="primary" type="button">Hemen Teklif İste</Button>
        <Button data-vehicle-detail-action="basket" fullWidth onClick={() => addToCart()} size="secondary" type="button" variant="outline">
          {added ? "Sepete Eklendi" : "Araç Sepetine Ekle"}
        </Button>
        <p aria-live="polite" className="sr-only">{added ? `${props.make} ${props.model} sepete eklendi.` : ""}</p>
      </div>
    </>
  );
}
